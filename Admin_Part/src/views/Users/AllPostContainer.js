...
class AllPostContainer extends Component {
  ...
  getPostList = () => {
    const apiUrl = `${config.Url}post/getAllPosts`;
    ...
    fetch(apiUrl, {
      ...
      .then((data) => {
        if (data.status === 200) {
          ...
          this.setState({ postLists: data.data });
          this.setState({ totalPost: data.total });
        } else {
  ...
  handleDelete = () => {
    const apiUrl = `${config.Url}post/${this.state.post_Id}`;
    const token = localStorage.getItem("token");
  ...
  handlePageChange(page) {
    ...
    const apiUrl = `${config.Url}post/getAllPosts?page=${
      page ? page : 1
      ...
      <div className="main-content-container p-4 container-fluid">
        ...
        <div className="AllUsersView_sect">
              ...
              <table className="table">
                ...
                <tbody>
                  {this.state.postLists &&
                    this.state.postLists.map((post, index) => (
                      <tr key={index}>
                        <td>
                          <div class="iagt">
                              ...
                              <img
                                src={
                                  post.user && post.user.profile_pic
                                    ? `https://halahomes.ca/uploads/${post.user.profile_pic}`
                                    : userPic
                                }
                              />
                            </div>
                            <div className="user_dt">
                              <h3>{post.user && post.user.full_name}</h3>
                              <p>
                                {" "}
                                {post.user &&
                                  post.user.details &&
                                  post.user.details.city}
                                {post.user &&
                                  post.user.details &&
                                  post.user.details.provience}
                              </p>
                            </div>
                          </div>
                        </td>
                        <td>{post.post_id}</td>
                        <td>
                          {moment(post.created_at).format("ddd, DD MMM YYYY")}
                        </td>
                        <td>
                          <div
                            className={
                        ...
                        <td>
                          <div className="action_btn">
                            <Link to="/admin/Users/AllPostView">
                              <button
                                className="btn"
                                onClick={() => this.postDetail(post.post_id)}
                              >
                                <svg
                            ...
                            <button
                              className="btn"
                              onClick={() => this.openDeleteModal(post.post_id)}
                            >
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="18"
          ...
          <div className="pagenation_item">
            <h3>
              {" "}
              Showing {this.state.postLists &&
                this.state.postLists.length} from {this.state.totalPost} data
            </h3>
