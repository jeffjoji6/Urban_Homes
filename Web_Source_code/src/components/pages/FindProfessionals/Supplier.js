...
const filteredUsers = users.filter((user) =>
  ...
  const fetchUser = async () => {
    try {
      ...
      const response = await api.get(`/users?category=Supplier&city=${city}&rating=${rating}&experience=${experience}`);
...
<img src={profilePic} className='h-14 object-cover rounded-full border-2 mr-2 border-slate-200'></img>
    ...
    <div className='border-0'> 
      <p className='text-3xl font-bold pt-12 text-gray-300'>No Suppliers for selected Filters</p>
      </div>)}
      </div>):(
        <div>
        {users[0] ? (<tbody>
