  ...
  const filteredUsers = users.filter((user) =>
    ...
    const fetchUser = async () => {
      try {
        ...
        const response = await api.get(`/users?category=Builder&city=${city}&rating=${rating}&experience=${experience}`);
...
<img src={profilePic} className='h-14 object-cover rounded-full border-2 mr-2 border-slate-200'></img>
            ...
            {[...Array(5)].map((star, index) => {
              ...
              return (
               ...
               <div> 
                 <button onClick={() => { setShowModal(true); setFullName(user.full_name); setSelectedId(user.user_id)}} className='transition-all hover:scale-105 md:h-full px-5 bg-transparent text-[#994B00] border  border-green-700 rounded-md font-bold text-sm md:text-xl'><i className="mr-2 text-[#994b00]  fas fa-envelope"></i>Send Message</button>
               </div>
             </div>
           </div>
    ...
    <div className='border-0'> 
      <p className='text-3xl font-bold pt-12 text-gray-300'>No Builders for selected Filters</p>
      </div>)}
      </div>):(
        <div>
        {users[0] ? (<tbody>
