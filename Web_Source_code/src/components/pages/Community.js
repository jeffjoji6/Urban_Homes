 ...
 //-----------------------------------FETCHING POSTS--------------------------------------
    ...
    const fetchUser = async () => {
      try {
        ...
        const recommended_topic= Cookies.get('recommended_topic')
        const response = await api.get(`/posts/${recommended_topic}?user_id=${uid}`);
    ...
    <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;700&display=swap"></link>
      <div className='flex flex-col w-full h-full  '>
        <div className="absolute items-center inset-0 h-[360px] md:h-[800px] bg-gradient-to-b from-black to-transparent opacity-85  -z-10"></div>
          <img src={head_image} alt= "pool-side home"className='absolute inset-0 h-[450px] md:h-[800px] w-full object-cover -z-20'></img>
        ...
        <div className='w-full md:flex mt-20 md:mt-32 flex-row'>
                            ...
                            <div>
                              ...
                              <div className="flex items-right space-x-4 mt-2">
                                  ...
                                  <div className="flex items-right">
                                    ...
                                    <span className="ml-2 text-xs mt">{people.comment_count}</span>
                                  </div>
          ...
          <form className=' w-full bg-transparent border-0' onSubmit={handleFormSubmit}>
              ...
              />
              {file && (<img  className="h-40 object-cover" src={URL.createObjectURL(preview)}></img>
)}
              <div className= 'md:w-2/6 md:mr-auto p-2'>
              <div  class="  text-white py-2 text-xl text-center w-full rounded-full">
              <div className=' sm:flex'>
                <label>
                  <img src={image} for="imageInput" className='cursor-pointer mr-3'></img>
                  <input type="file" id="imageInput" onChange={handleFileChange} accept="image/* " class="hidden"></input>
                </label>
                <label>
                  <img src={video} for="videoInput" className='cursor-pointer mr-3' ></img>
                  <input type="file" id="videoInput"  onChange={handleFileChange} accept="video/* " class="hidden"></input>
                </label>
            ...
            <h2 className=' justify-center text-center md:text-3xl font-semibold mt-20 md:hidden text-2xl'>Commmunity Posts</h2>
                ...
                <div className='m-8 shadow-lg  hover:scale-[1.01] transition-all flex border-2 border-slate-100 p-6 pt-6 rounded-md text-lg'>
                  <Link className="w-full" to={`/post/?post_id=${user.post_id}`} ><div className=' justify-between space-y-8 w-full'>
                    <div className='flex-col'>
                      <div className='flex'>
                        <img src={`../uploads/${user.profile_pic}`} className='h-14 w-14 object-cover rounded-full border-2 mr-4 border-slate-200'></img>
                        <div className='flex-col'>
